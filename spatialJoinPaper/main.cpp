...
namespace osc {
  ...
  extern "C" int main(int ac, char **av)
  {
    try {
      ...
      // output file
      std::ofstream outFile("RToutput_self_intersection.txt");
      std::ofstream outFile_pairwise("RToutput_pairwise_intersection.txt"); 
      ...
      for (fs::directory_entry& organ_path : fs::directory_iterator(body_path)) 
      {
        ...
        std::vector<fs::directory_entry> anatomical_structures;

        // Collect all entries
        for (const fs::directory_entry& entry : fs::directory_iterator(organ_path)) {
            anatomical_structures.push_back(entry);
        }

        // Perform pairwise comparisons between anatomical structures (no self-self intersection)
        for (int i = 0; i < anatomical_structures.size(); i++) {
          for (int j = i + 1; j < anatomical_structures.size(); j++) {
            std::string as_path1 = anatomical_structures[i].path().string();
            std::string as_path2 = anatomical_structures[j].path().string();
            TriangleMesh model1;
            TriangleMesh model2;
            model1.addTriangleMesh(as_path1);
            model2.addTriangleMesh(as_path2);

            if (model1.index.size() < face_threshold || model2.index.size() < face_threshold) continue;

            outFile_pairwise << "Model1: " << as_path1 << std::endl;
            outFile_pairwise << "Model2: " << as_path2 << std::endl;
            outFile_pairwise << "Model1 size: " << model1.index.size() << std::endl;
            outFile_pairwise << "Model2 size: " << model2.index.size() << std::endl;

            duration = t33 - t3;
            std::cout << "Model loading time: " << duration.count() << " seconds" << std::endl;
            outFile_pairwise << "Model loading time: " << duration.count() << " seconds" << std::endl;

            auto t4 = std::chrono::high_resolution_clock::now();
            sjrenderer->render(model1, model2, launchDim, outFile_pairwise);
            ...
            outFile_pairwise << "RT download result: " << duration.count() << " seconds" << std::endl;
            std::cout << "compute results: " << duration.count() << " seconds" << std::endl; 

            std::cout << "\n*******************************************************\n" << std::endl;
            if (is_collided) {
              std::cout << "Result: two models are collided. \n" << std::endl;
              outFile_pairwise << "Result: two models are collided. \n" << std::endl;
